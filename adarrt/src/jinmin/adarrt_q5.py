...
class AdaRRT():
    ...
    def _get_random_sample_near_goal(self):
        return self.goal.state + np.random.uniform(-0.05, 0.05, len(self.goal.state))
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        new_sample = neighbor.state + self.step_size * unit_vector
        if self._check_for_collision(new_sample):
            return None
        return neighbor.add_child(new_sample)
